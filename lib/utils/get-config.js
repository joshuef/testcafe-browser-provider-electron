...
exports.default = function (id, mainPath) {
    if ((0, _fs.statSync)(mainPath).isDirectory()) mainPath = _path2.default.join(mainPath, _constants2.default.configFileName);

    var mainDir = _path2.default.dirname(mainPath);
    ...
    try {
        ...
        // do nothing
        var configString = (0, _fs.readFileSync)(mainPath).toString();

        config = JSON.parse(configString);
    }

    if (config.appPath) {
        if (!(0, _isAbsolute2.default)(config.appPath)) config.appPath = _path2.default.resolve(mainDir, config.appPath);
